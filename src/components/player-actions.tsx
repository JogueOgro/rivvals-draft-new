...
const PlayerActions = ({ row, data }: { data: IPlayer[], row: { original: IPlayer } }) => {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>, playerId: string) => {
    try {
      ...
      if (file.size > 400 * 1024) {
        window.alert('O arquivo é muito grande. Por favor, selecione um arquivo menor que 400KB.');
        return;
      }
      ...
      window.alert('Ocorreu um erro ao processar o arquivo. Por favor, tente novamente.');
    }
  };
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuItem onClick={() => deletePlayersUseCase.execute([row.original.id!])}>
